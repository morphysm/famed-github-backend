...
type safeWrappedIssue struct {
	sync.RWMutex
	enrichedIssue map[int]model2.EnrichedIssue
}
...
func (gH *githubHandler) loadIssues(ctx context.Context, owner string, repoName string) (map[int]model2.EnrichedIssue, error) {
	...
	wg := sync.WaitGroup{}
	safeIssues := safeWrappedIssue{enrichedIssue: make(map[int]model2.EnrichedIssue, len(issuesResponse))}
